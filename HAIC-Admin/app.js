...
function renderSiteRecords(records) {
  ...
  const defaultRecords = defaultMonth
    ? records.filter(r => monthKeyOf(r.date) === defaultMonth)
    : records;
  const totalEligible = defaultRecords.filter(r => r.eligible && r.status==='approved').reduce((s,r) => s + (r.amount||0), 0);
...
window.filterSiteRecords = function() {
  ...
  const totalEligible = filtered.filter(r => r.eligible && r.status==='approved')
