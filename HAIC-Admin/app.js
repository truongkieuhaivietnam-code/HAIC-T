...
// hướng dẫn) để lấy TOKEN. Sau đó tạo RIÊNG 1 nhóm cho MỖI QUỐC GIA (Cam/
// Lào/Việt), thêm bot vào từng nhóm, gửi 1 tin bất kỳ trong nhóm, rồi mở
// link sau (thay TOKEN thật vào) để lấy CHAT_ID (số âm, VD -1001234567890)
// của TỪNG nhóm:
...
// SỬA: trước đây dùng 1 CHAT_ID chung cho cả công ty — giờ mỗi quốc gia
// có 1 nhóm riêng, không còn dồn chung Cam/Lào/Việt vào 1 nhóm nữa.
const TELEGRAM_CHAT_IDS = {
  Cambodia: '-5572709840',
  Laos:     '-5441742692',
  Vietnam:  'DAN_CHAT_ID_VIETNAM_VAO_DAY',
};

// country: 'Cambodia' | 'Laos' | 'Vietnam' — gửi vào ĐÚNG nhóm quốc gia đó.
// Nếu không xác định được quốc gia (dữ liệu cũ/thiếu), bỏ qua an toàn thay
// vì gửi nhầm vào 1 nhóm mặc định nào đó.
async function sendTelegramNotification(country, text) {
  if (!TELEGRAM_BOT_TOKEN || TELEGRAM_BOT_TOKEN.startsWith('DAN_')) {
    console.warn('Telegram chưa được cấu hình (thiếu TOKEN) — bỏ qua gửi thông báo.');
    return;
  }
  const chatId = TELEGRAM_CHAT_IDS[country];
  if (!chatId || chatId.startsWith('DAN_')) {
    console.warn(`Telegram chưa cấu hình CHAT_ID cho quốc gia "${country}" — bỏ qua gửi thông báo.`);
    return;
  }
  try {
...
const LANGS = {
  en: {
    ...
    leave_session_lbl: 'Leave Session',
    leave_session_full: 'Full Day',
    leave_session_morning: 'Morning Only (0.5 day)',
    leave_session_afternoon: 'Afternoon Only (0.5 day)',
    leave_total_entitled: 'Total Entitled',
    leave_used: 'Days Used',
    leave_remaining: 'Days Remaining',
    ...
    site_logic_hint:  'Eligible if: total hours worked (End − Start) > 4 hours',
    // OT
  ...
  vi: {
    ...
    leave_session_lbl: 'Buổi nghỉ',
    leave_session_full: 'Cả ngày',
    leave_session_morning: 'Buổi sáng (0.5 ngày)',
    leave_session_afternoon: 'Buổi chiều (0.5 ngày)',
    leave_total_entitled: 'Tổng ngày phép được hưởng',
    leave_used: 'Đã nghỉ',
    leave_remaining: 'Còn lại',
    ...
    site_logic_hint:  'Đủ điều kiện nếu: tổng số giờ làm việc (Giờ ra − Giờ vào) lớn hơn 4 tiếng',
    // OT
...
const EXTERNAL_APPS = [
  ...
  { icon: '🧫', label: 'Grout Test',      url: 'https://truongkieuhaivietnam-code.github.io/HAIC-T/grout-test.html' },
...
async function loadLeave() {
  ...
  try {
    ...
    // TOÀN BỘ nhân sự đang hoạt động (không lọc theo vai trò) — dùng cho
    // bảng "Số dư phép — tất cả nhân sự" bên dưới, để Admin/Quản lý quốc
    // gia cũng hiện đủ dù chưa từng có đơn xin nghỉ nào.
    ...
    // Lấy số dư phép hiện tại của TẤT CẢ nhân sự (không chỉ người có đơn)
    // — dùng để tính "Tổng ngày phép / Đã nghỉ / Còn lại" cho mỗi người.
    const uids = [...new Set([...allEmployees.map(e => e.uid), ...requests.map(r => r.uid)].filter(Boolean))];
    const balanceMap = {};
    await Promise.all(uids.map(async uid => {
      try {
        const balSnap = await col.leaveBalance().doc(uid).get();
        balanceMap[uid] = balSnap.exists ? (balSnap.data().balance || 0) : 0;
      } catch(e) { balanceMap[uid] = 0; }
    }));

    _leaveRequestsCache = requests;
    _leaveBalanceMapCache = balanceMap;
    _leaveAllEmployeesCache = allEmployees;
...
function renderLeave(requests, balanceMap) {
  ...
  const allEmployees = _leaveAllEmployeesCache || [];

  const roleLabelMap = {
    superadmin: lang==='vi' ? 'Quản trị tối cao' : 'Super Admin',
    admin: lang==='vi' ? 'Admin' : 'Admin',
    country_manager: lang==='vi' ? 'Quản lý quốc gia' : 'Country Manager',
    employee: lang==='vi' ? 'Nhân viên' : 'Employee',
  };

  const allStaffRows = allEmployees
    .slice()
    .sort((a,b) => (a.name||'').localeCompare(b.name||''))
    .map(e => {
      const remaining = e.uid in balanceMap ? balanceMap[e.uid] : 0;
      const used = usedByUid[e.uid] || 0;
      const total = remaining + used;
      return `<tr>
        <td><strong>${e.name || '–'}</strong></td>
        <td>${COUNTRY_FLAG[e.country] || ''} ${e.country || ''}</td>
        <td>${roleLabelMap[e.role] || e.role || '–'}</td>
        <td>${total}</td>
        <td>${used}</td>
        <td>${remaining}</td>
        <td>${canManageCountry(e.country) ? `<button class="btn btn-sm btn-outline" onclick="openEditLeaveBalanceForEmployee('${e.uid}','${(e.name||'').replace(/'/g,"\\'")}')" title="${lang==='vi'?'Sửa số ngày phép':'Edit leave balance'}">🌴</button>` : ''}</td>
      </tr>`;
    }).join('');
  ...
  $('page-leave').innerHTML = `
    ...
    <div class="card">
      <h3 style="margin:0 0 10px;">${lang==='vi' ? 'Số dư phép — tất cả nhân sự' : 'Leave Balance — All Staff'}</h3>
      <p class="small-muted" style="margin:0 0 10px;">${lang==='vi' ? 'Hiện đầy đủ mọi nhân sự đang hoạt động (kể cả Admin/Quản lý quốc gia), không chỉ người đã từng có đơn xin nghỉ.' : 'Shows every active staff member (including Admin/Country Manager), not just those who have submitted a leave request.'}</p>
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>${t('att_employee')}</th><th>${t('emp_country')}</th><th>${lang==='vi'?'Vai trò':'Role'}</th>
            <th>${t('leave_total_entitled')}</th><th>${t('leave_used')}</th><th>${t('leave_remaining')}</th><th>${t('emp_actions')}</th>
          </tr></thead>
          <tbody>${allStaffRows || `<tr><td colspan="7"><div class="empty-state"><h4>${lang==='vi'?'Không có nhân sự':'No staff'}</h4></div></td></tr>`}</tbody>
        </table>
      </div>
    </div>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            ...
            <th>${t('leave_from')}</th><th>${t('leave_to')}</th><th>${t('leave_session_lbl')}</th>
            <th>${t('leave_days')}</th><th>${t('leave_type')}</th>
            <th>${t('leave_total_entitled')}</th><th>${t('leave_used')}</th><th>${t('leave_remaining')}</th>
            <th>${t('leave_reason')}</th><th>${t('leave_status')}</th><th>${t('emp_actions')}</th>
          </tr></thead>
          <tbody id="leave-tbody">
            ${requests.map(r => leaveRow(r, balanceMap, usedByUid)).join('')}
          </tbody>
        </table>
      </div>
    </div>
...
function leaveSessionLabel(r) {
  if (r.session === 'morning') return lang==='vi' ? 'Sáng' : 'AM';
  if (r.session === 'afternoon') return lang==='vi' ? 'Chiều' : 'PM';
  return lang==='vi' ? 'Cả ngày' : 'Full';
}

function leaveRow(r, balanceMap, usedByUid) {
  const statusBadge = {
  ...
  const remaining = balanceMap && r.uid in balanceMap ? balanceMap[r.uid] : '–';
  const used = usedByUid && r.uid in usedByUid ? usedByUid[r.uid] : 0;
  const totalEntitled = (typeof remaining === 'number') ? (remaining + used) : '–';
...
window.saveAttendance = async function() {
  ...
  try {
    ...
    // Chỉ báo Telegram khi NHÂN VIÊN tự khai (cần admin để ý duyệt) — admin
    // tự ghi cho người khác thì không cần báo (đã là người chủ động biết).
    if (!isAdminLike) {
      const typeLabel = type === 'late_approved' ? (lang==='vi'?'Đi trễ có phép':'Approved late') : (lang==='vi'?'Vắng không phép':'Unauthorized absence');
      sendTelegramNotification(
        state.userProfile.country,
        lang==='vi'
          ? `⏱️ <b>Khai báo chuyên cần mới (chờ duyệt)</b>\n👤 ${empName}\n📅 ${date} — ${typeLabel} (${mins} phút)\n💰 Phạt dự kiến: $${penalty.toFixed(2)}`
          : `⏱️ <b>New Attendance Self-report (Pending)</b>\n👤 ${empName}\n📅 ${date} — ${typeLabel} (${mins} min)\n💰 Estimated penalty: $${penalty.toFixed(2)}`
      );
    }
...
window.approveAttendance = async function(id) {
  ...
  try {
    const docSnap = await col.attendance().doc(id).get();
    const rec = docSnap.exists ? docSnap.data() : null;
    ...
    if (rec) {
      sendTelegramNotification(
        rec.country,
        lang==='vi'
          ? `✅ <b>Đã duyệt bản ghi chuyên cần</b>\n👤 ${rec.employeeName || ''}\n📅 ${rec.date} — Phạt: $${(rec.penalty||0).toFixed(2)} (đã tính vào lương)`
          : `✅ <b>Attendance Record Approved</b>\n👤 ${rec.employeeName || ''}\n📅 ${rec.date} — Penalty: $${(rec.penalty||0).toFixed(2)} (now counted toward payroll)`
      );
    }
  ...
  try {
    ...
    if (rec) {
      sendTelegramNotification(
        rec.country,
        lang==='vi'
          ? `❌ <b>Đã từ chối bản ghi chuyên cần</b>\n👤 ${rec.employeeName || ''}\n📅 ${rec.date}`
          : `❌ <b>Attendance Record Rejected</b>\n👤 ${rec.employeeName || ''}\n📅 ${rec.date}`
      );
    }
...
window.saveViolation = async function() {
  ...
  try {
    ...
    sendTelegramNotification(
      emp?.country,
      lang==='vi'
        ? `⚠️ <b>Vi phạm mới được ghi nhận</b>\n👤 ${emp?.name || ''}\n📅 ${date}\n📋 Loại: ${typeName || ''}\n💰 Phạt: $${penalty.toFixed(2)}`
        : `⚠️ <b>New Violation Logged</b>\n👤 ${emp?.name || ''}\n📅 ${date}\n📋 Type: ${typeName || ''}\n💰 Penalty: $${penalty.toFixed(2)}`
    );
...
// ── PAYROLL ───────────────────────────────────────────────────
// SỬA LỖI: trước đây MỌI hàm liên quan bảng lương (tạo/khóa/mở khóa/sửa)
// đều tự tính "tháng hiện tại" ĐỘC LẬP với nhau — khiến không thể nào xem
// hay tạo bảng lương cho tháng TRƯỚC (VD đã sang tháng 8 thì không tạo
// được bảng lương tháng 7 nữa). Giờ dùng CHUNG 1 biến trạng thái, có thể
// chọn tháng khác qua ô chọn tháng trên giao diện.
let CURRENT_PAYROLL_MONTH = null;
function defaultMonthStr(){
  const now = new Date();
  return `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
}
function getPayrollMonth(){
  return CURRENT_PAYROLL_MONTH || defaultMonthStr();
}

async function loadPayroll(month) {
  showLoader();
  try {
    month = month || getPayrollMonth();
    CURRENT_PAYROLL_MONTH = month;
...
function renderPayroll(employees, existing, month) {
  ...
  $('page-payroll').innerHTML = `
    <div class="page-header">
      <div><h2>Payroll</h2><p>Month: <input type="month" id="payrollMonthPicker" value="${month}" onchange="loadPayroll(this.value)" style="padding:4px 8px;border:1px solid var(--line,#ccc);border-radius:4px;"></p></div>
      <div class="page-actions">
        ${!isLocked ? `
...
async function loadMyLeave() {
  ...
  const totalEntitled = balance + usedDays;

  const sessionLabel = (r) => r.session === 'morning' ? t('leave_session_morning').replace(' (0.5 day)','').replace(' (0.5 ngày)','')
    : r.session === 'afternoon' ? t('leave_session_afternoon').replace(' (0.5 day)','').replace(' (0.5 ngày)','')
    : t('leave_session_full');

  $('page-my-leave').innerHTML = `
    <div class="page-header">
      <div><h2>${t('page_my_leave')}</h2></div>
      <div class="page-actions">
        <button class="btn btn-primary" onclick="openRequestLeave()">${t('leave_request_btn')}</button>
      </div>
    </div>
    <div class="stats-grid">
      <div class="stat-card navy"><div class="stat-icon">📅</div><div class="stat-value">${totalEntitled}</div><div class="stat-label">${t('leave_total_entitled')}</div></div>
      <div class="stat-card gold"><div class="stat-icon">🌴</div><div class="stat-value">${usedDays}</div><div class="stat-label">${t('leave_used')}</div></div>
      <div class="stat-card green"><div class="stat-icon">✅</div><div class="stat-value">${balance}</div><div class="stat-label">${t('leave_remaining')}</div></div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>${t('leave_from')}</th><th>${t('leave_to')}</th><th>${t('leave_session_lbl')}</th><th>${t('leave_days')}</th>
            <th>${t('leave_type')}</th><th>${t('leave_reason')}</th><th>${t('leave_status')}</th>
          </tr></thead>
          <tbody>
            ${requests.length ? requests.map(r=>`
              <tr>
                <td>${r.from}</td><td>${r.to}</td><td>${sessionLabel(r)}</td><td>${r.days}</td>
                <td>${r.leaveType==='paid'?`🟢 ${t('leave_paid')}`:`🔴 ${t('leave_unpaid')}`}</td>
                <td>${r.reason||''}</td>
            ...
            : `<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">🌴</div><h4>${t('my_no_leave')}</h4></div></td></tr>`}
          </tbody>
    ...
    <div class="modal-overlay" id="modal-my-leave">
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label>${t('leave_session_lbl')}</label>
            <select class="form-control" id="leave-session" onchange="onLeaveSessionChange()">
              <option value="full">${t('leave_session_full')}</option>
              <option value="morning">${t('leave_session_morning')}</option>
              <option value="afternoon">${t('leave_session_afternoon')}</option>
            </select>
          </div>
          <div class="form-row">
            <div class="form-group"><label>${t('leave_from_date')}</label><input class="form-control" type="date" id="leave-from" onchange="onLeaveFromChange()"></div>
            <div class="form-group" id="leave-to-group"><label>${t('leave_to_date')}</label><input class="form-control" type="date" id="leave-to"></div>
          </div>
...
window.onLeaveSessionChange = function() {
  const session = $('leave-session')?.value;
  const toGroup = $('leave-to-group');
  if (session === 'morning' || session === 'afternoon') {
    // Nghỉ nửa buổi chỉ áp dụng cho ĐÚNG 1 NGÀY — ẩn "Đến ngày", tự khóa
    // bằng đúng "Từ ngày".
    toGroup.style.display = 'none';
    if ($('leave-from').value) $('leave-to').value = $('leave-from').value;
  } else {
...
window.onLeaveFromChange = function() {
  const session = $('leave-session')?.value;
  if (session === 'morning' || session === 'afternoon') {
    $('leave-to').value = $('leave-from').value;
  }
};

window.openRequestLeave = function() {
  // Reset về mặc định mỗi lần mở lại form
  const sessionSel = $('leave-session');
  if (sessionSel) sessionSel.value = 'full';
  onLeaveSessionChange();
  openModal('modal-my-leave');
};

window.submitLeaveRequest = async function() {
  const from    = $('leave-from')?.value;
  let   to      = $('leave-to')?.value;
  const session = $('leave-session')?.value || 'full';
  const reason  = $('leave-reason')?.value.trim();

  // Validate
  if (!from) {
    toast(lang==='vi' ? 'Vui lòng chọn ngày nghỉ.' : 'Please select a leave date.', 'error');
    return;
  }
  // Nghỉ nửa buổi: luôn đúng 1 ngày, "đến ngày" = "từ ngày" — không phụ
  // thuộc vào việc người dùng có kịp thấy ô "Đến ngày" bị ẩn hay không.
  if (session === 'morning' || session === 'afternoon') {
    to = from;
  }
  if (!to) {
    toast(lang==='vi' ? 'Vui lòng chọn ngày kết thúc.' : 'Please select an end date.', 'error');
    return;
  }
  ...
  // Nghỉ 1 buổi (sáng/chiều) = 0.5 ngày; nghỉ cả ngày tính theo số ngày
  // thực tế trong khoảng đã chọn (như cũ).
  const days = (session === 'morning' || session === 'afternoon')
    ? 0.5
    : Math.ceil((toDate - fromDate) / 86400000) + 1;
  ...
  try {
    await col.leave().add({
      ...
      country:      state.userProfile.country || '',
      from, to, days, session, reason, leaveType,
      status:    'pending',
      createdAt: firebase.firestore.FieldValue.serverTimestamp()
    });

    const sessionLabelVi = session==='morning' ? ' (buổi sáng)' : session==='afternoon' ? ' (buổi chiều)' : '';
    const sessionLabelEn = session==='morning' ? ' (morning)' : session==='afternoon' ? ' (afternoon)' : '';

    // Gửi thông báo Telegram — riêng theo đúng nhóm quốc gia của người
    // xin nghỉ (không còn dồn chung Cam/Lào/Việt vào 1 nhóm nữa).
    sendTelegramNotification(
      state.userProfile.country,
      lang==='vi'
        ? `🌴 <b>Đơn xin nghỉ phép mới</b>\n👤 ${state.userProfile.name || ''} (${state.userProfile.country || ''})\n📅 ${from} → ${to} (${days} ngày${sessionLabelVi}, ${leaveType==='paid'?'có lương':'không lương'})\n📝 Lý do: ${reason}`
        : `🌴 <b>New Leave Request</b>\n👤 ${state.userProfile.name || ''} (${state.userProfile.country || ''})\n📅 ${from} → ${to} (${days} day(s)${sessionLabelEn}, ${leaveType})\n📝 Reason: ${reason}`
    );

    toast(
      lang==='vi'
        ? `✅ Đã gửi đơn xin nghỉ ${days} ngày${sessionLabelVi} (${leaveType==='paid'?'có lương':'không lương'}). Chờ phê duyệt.`
        : `✅ Leave request submitted: ${days} day(s)${sessionLabelEn} (${leaveType}). Pending approval.`,
      'success'
    );
...
window.saveOT = async function() {
  ...
  try {
    // SỬA LỖI NGHIÊM TRỌNG: trước đây ghi vào collection "ot" — nhưng
    // trang "Quản lý OT" và cả bảng lương đều chỉ đọc từ "ot_requests".
    // Nghĩa là mọi bản ghi admin tự log qua đây từ trước tới giờ đều
    // KHÔNG hiện ở đâu cả và KHÔNG được tính vào lương. Giờ ghi đúng vào
    // "ot_requests", cùng tên field (employeeId) và trạng thái admin tự
    // ghi = tin cậy ngay ("approved"), khớp đúng những gì trang Quản lý
    // OT và bảng lương đang thực sự đọc.
    await col.otRequests().add({
      employeeId: uid, date, hours, dailyRate, hourlyRate, otPay,
...
window.generatePayroll = async function() {
  ...
  try {
    ...
    const [attSnap, violSnap, otSnap, holidayOTSnap] = await Promise.all([
      ...
      col.violations()
        .where('date', '>=', monthStart).where('date', '<=', monthEnd).get(),
      col.otRequests().where('otType', '==', 'normal').where('status','==','approved')
        .where('date', '>=', monthStart).where('date', '<=', monthEnd).get(),
      col.otRequests().where('otType', '==', 'holiday').where('status','==','approved')
        .where('date', '>=', monthStart).where('date', '<=', monthEnd).get()
    ]);
...
window.editSiteRecord = async function(id) {
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            ${amPmTimeInputHtml('es-start', rec.startTime||'07:30', 'onchange="recalcSiteEdit()"')}
          </div>
          <div class="form-group">
            <label>${t('site_end')}</label>
            ${amPmTimeInputHtml('es-end', rec.endTime||'14:00', 'onchange="recalcSiteEdit()"')}
          </div>
        </div>
        <div class="form-group">
          <label>${lang==='vi'?'Quãng đường 1 chiều (Lab → Site, km)':'One-way distance (Lab → Site, km)'}</label>
          <input class="form-control" type="number" id="es-distance" value="${rec.distanceKm != null ? rec.distanceKm : ''}" min="0" step="0.1" oninput="recalcSiteEdit()">
        </div>
  ...
  overlay.addEventListener('click', e => { if (e.target===overlay) overlay.remove(); });
  initAmPmTimeInputs();
  setTime12('es-start', rec.startTime||'07:30');
  setTime12('es-end', rec.endTime||'14:00');
  recalcSiteEdit();
};

window.recalcSiteEdit = function() {
  const start    = getTime12('es-start');
  const end      = getTime12('es-end');
  const location = $('es-location')?.value || 'Phnom Penh';
  const distance = $('es-distance')?.value || '';
  const eligible = calcSiteEligible(start, end);
  const rate     = SITE_RATES[location] || 5;
  const fuel     = calcFuelAllowance(distance);
  const el       = $('es-result');
  if (!el) return;
  let html = eligible
  ...
  if (distance) {
    if (fuel === null) {
      html += `<br>⛽ ${lang==='vi'?'>50km — thanh toán riêng':'>50km — settle separately'}`;
    } else if (fuel > 0) {
      html += `<br>⛽ ${lang==='vi'?'Phụ cấp xăng':'Fuel'}: <strong class="payroll-positive">+$${fuel}</strong>`;
    }
  }
  el.innerHTML = html;
};
...
window.saveSiteEdit = async function(id) {
  ...
  const distance = $('es-distance')?.value || '';
  const eligible = calcSiteEligible(start, end);
  const fuelAllowance = calcFuelAllowance(distance);
  const fuelOver50km = fuelAllowance === null;
  const amount   = (eligible ? (SITE_RATES[location]||5) : 0) + (fuelOver50km ? 0 : (fuelAllowance || 0));
  showLoader();
  try {
    await col.siteRecords().doc(id).update({
      ...
      fuelAllowance: fuelOver50km ? 0 : (fuelAllowance || 0),
      fuelOver50km,
      amount,
      editedBy:  state.userProfile.uid,
      editedAt:  firebase.firestore.FieldValue.serverTimestamp(),
...
// ── Month filter helpers (dùng chung cho OT & Site Records) ──────────
// key: 'YYYY-MM' lấy từ field date ('YYYY-MM-DD')
function monthKeyOf(dateStr) { return (dateStr || '').slice(0, 7); }

function monthLabelOf(key) {
  if (!key) return '';
  const [y, m] = key.split('-');
  if (lang === 'vi') return `Tháng ${parseInt(m,10)}/${y}`;
  const names = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  return `${names[parseInt(m,10)-1]} ${y}`;
}

// Trả về { optionsHtml, defaultMonth } — defaultMonth là tháng hiện tại
// nếu có dữ liệu, ngược lại rỗng (nghĩa là "Tất cả").
function buildMonthFilterOptions(records) {
  const months = [...new Set(records.map(r => monthKeyOf(r.date)).filter(Boolean))]
    .sort().reverse();
  const currentMonth = new Date().toISOString().slice(0, 7);
  const defaultMonth = months.includes(currentMonth) ? currentMonth : '';
  const optionsHtml = months.map(m =>
    `<option value="${m}" ${m === defaultMonth ? 'selected' : ''}>${monthLabelOf(m)}</option>`
  ).join('');
  return { optionsHtml, defaultMonth, months };
}
...
async function loadOTManage() {
  ...
  try {
    ...
    // Sắp xếp theo ngày gần nhất trước — làm ở client để tránh cần
    // composite index (đặc biệt query của Employee không có orderBy).
...
function renderOTManage(records) {
  ...
  const { optionsHtml: otMonthOptionsHtml, defaultMonth } = buildMonthFilterOptions(records);
  const defaultRecords = defaultMonth
    ? records.filter(r => monthKeyOf(r.date) === defaultMonth)
    : records;
  const totalOTPay = defaultRecords.filter(r => r.status === 'approved')
    .reduce((s,r) => s + (r.otPay||0), 0);
  const pendingOTCount = defaultRecords.filter(r => r.status === 'pending').length;

  $('page-ot-manage').innerHTML = `
    <div class="page-header">
      <div>
        <h2>${t('page_ot_manage')}</h2>
        <p>
          <span id="ot-record-count">${defaultRecords.length}</span> ${lang==='vi'?'bản ghi':'records'}
          &nbsp;·&nbsp; ${lang==='vi'?'Tổng tiền OT đã duyệt':'Total approved OT pay'}:
          <strong class="payroll-positive" id="ot-total-pay">$${totalOTPay.toFixed(2)}</strong>
          <span id="ot-pending-count" style="color:var(--amber);${pendingOTCount>0?'':'display:none;'}">
            &nbsp;·&nbsp; ${pendingOTCount} ${lang==='vi'?'chờ duyệt':'pending'}
          </span>
        </p>
      </div>
      <div class="page-actions">
    ...
    <div class="filter-bar">
      <select class="form-control" id="ot-filter-month" onchange="filterOTRecords()">
        <option value="">${lang==='vi'?'Tất cả các tháng':'All Months'}</option>
        ${otMonthOptionsHtml}
      </select>
      ...
      <select class="form-control" id="ot-filter-employee" onchange="filterOTRecords()">
        <option value="">${lang==='vi'?'Tất cả nhân viên':'All Staff'}</option>
        ${(state.cache.employees || []).slice().sort((a,b)=>(a.name||'').localeCompare(b.name||'')).map(e=>`<option value="${e.uid}">${e.name}</option>`).join('')}
      </select>
    </div>
    ...
    <div class="card">
      <div class="table-wrap">
        <table id="ot-table">
          ...
          <tbody id="ot-tbody">
            ${renderOTRows(defaultRecords, canAdmin)}
          </tbody>
        </table>
      </div>
    </div>
...
window.filterOTRecords = function() {
  const month = $('ot-filter-month')?.value || '';
  const status = $('ot-filter-status')?.value || '';
  const type = $('ot-filter-type')?.value || '';
  const employee = $('ot-filter-employee')?.value || '';
  let filtered = _otRecordsCache;
  if (month) filtered = filtered.filter(r => monthKeyOf(r.date) === month);
  if (status) filtered = filtered.filter(r => r.status === status);
  if (type) filtered = filtered.filter(r => (r.otType || 'normal') === type);
  if (employee) filtered = filtered.filter(r => r.employeeId === employee);
  const canAdmin = isAdmin() || isCountryManager();
  const tbody = $('ot-tbody');
  if (tbody) tbody.innerHTML = renderOTRows(filtered, canAdmin);

  // Cập nhật tổng tiền OT đã duyệt + số bản ghi + số chờ duyệt theo bộ lọc hiện tại
  const totalOTPay = filtered.filter(r => r.status === 'approved')
    .reduce((s,r) => s + (r.otPay||0), 0);
  const pendingCount = filtered.filter(r => r.status === 'pending').length;
  const totalEl = $('ot-total-pay');
  if (totalEl) totalEl.textContent = `$${totalOTPay.toFixed(2)}`;
  const countEl = $('ot-record-count');
  if (countEl) countEl.textContent = filtered.length;
  const pendingEl = $('ot-pending-count');
  if (pendingEl) {
    if (pendingCount > 0) {
      pendingEl.style.display = '';
      pendingEl.innerHTML = `&nbsp;·&nbsp; ${pendingCount} ${lang==='vi'?'chờ duyệt':'pending'}`;
    } else {
...
// SỬA: trước đây yêu cầu đúng khung giờ cố định (vào ≤08:00 VÀ ra ≥13:00)
// — giờ đổi thành TỔNG SỐ GIỜ làm việc (End − Start) phải LỚN HƠN 4 tiếng,
// linh hoạt hơn cho các ca giờ giấc khác nhau thực tế ngoài công trường.
// ── Bộ chọn giờ tự dựng — luôn hiện AM/PM ──────────────────────────────
// Bộ chọn giờ gốc của trình duyệt (<input type="time">) hiển thị SA/CH
// hay AM/PM tùy theo NGÔN NGỮ HỆ ĐIỀU HÀNH, không sửa được đáng tin cậy
// bằng thuộc tính lang. Tự dựng 3 ô riêng: giờ (1-12) + phút (00-59) +
// AM/PM — luôn cố định tiếng Anh, không phụ thuộc máy ai đang dùng.
function amPmTimeInputHtml(id, defaultTime, onEvt){
  return `<span class="ampm-time-picker" style="display:inline-flex;gap:3px;">
    <select class="form-control" id="${id}_hour" style="width:56px;padding:6px 2px;" ${onEvt}></select>
    <select class="form-control" id="${id}_min" style="width:56px;padding:6px 2px;" ${onEvt}></select>
    <select class="form-control" id="${id}_ampm" style="width:62px;padding:6px 2px;" ${onEvt}>
      <option value="AM">AM</option><option value="PM">PM</option>
    </select>
  </span>`;
}
function initAmPmTimeInputs(){
  document.querySelectorAll('[id$="_hour"]').forEach(hourSel=>{
    const id = hourSel.id.replace(/_hour$/,'');
    const minSel = $(id+'_min');
    if(!minSel || hourSel.options.length>0) return;
    for(let h=1;h<=12;h++){ const o=document.createElement('option'); o.value=String(h); o.textContent=String(h); hourSel.appendChild(o); }
    for(let m=0;m<60;m++){ const o=document.createElement('option'); o.value=String(m).padStart(2,'0'); o.textContent=String(m).padStart(2,'0'); minSel.appendChild(o); }
  });
}
function getTime12(id){
  const hourEl=$(id+'_hour'), minEl=$(id+'_min'), ampmEl=$(id+'_ampm');
  if(!hourEl || !hourEl.value) return '';
  let h = parseInt(hourEl.value,10);
  const ampm = ampmEl.value||'AM';
  if(ampm==='AM') h = (h===12)?0:h; else h = (h===12)?12:h+12;
  return `${String(h).padStart(2,'0')}:${minEl.value||'00'}`;
}
function setTime12(id, hhmm){
  const hourEl=$(id+'_hour'), minEl=$(id+'_min'), ampmEl=$(id+'_ampm');
  if(!hourEl) return;
  if(!hhmm){ hourEl.value='7'; minEl.value='00'; ampmEl.value='AM'; return; }
  let [h,m] = hhmm.split(':').map(s=>parseInt(s,10));
  const ampm = h>=12 ? 'PM':'AM';
  let h12 = h%12; if(h12===0) h12=12;
  hourEl.value=String(h12); minEl.value=String(m).padStart(2,'0'); ampmEl.value=ampm;
}
...
function calcSiteEligible(startTime, endTime) {
  ...
  if (endMins < startMins) endMins += 24 * 60; // qua ngày hôm sau (ca đêm) — vẫn tính đúng số giờ
...
function renderSiteRecords(records) {
  ...
  const { optionsHtml: siteMonthOptionsHtml, defaultMonth } = buildMonthFilterOptions(records);
  const defaultRecords = defaultMonth
    ? records.filter(r => monthKeyOf(r.date) === defaultMonth)
    : records;
  const totalEligible = defaultRecords.filter(r => r.status==='approved').reduce((s,r) => s + (r.amount||0), 0);
  const pendingCount  = defaultRecords.filter(r => r.status === 'pending').length;
  ...
  $('page-site').innerHTML = `
    <div class="page-header">
      <div>
        <h2>${t('page_site')}</h2>
        <p>
          <span id="site-record-count">${defaultRecords.length}</span> ${lang==='vi'?'bản ghi':'records'}
          &nbsp;·&nbsp; ${lang==='vi'?'Tổng phụ cấp đã duyệt':'Total approved allowance'}:
          <strong class="payroll-positive" id="site-total-eligible">$${totalEligible.toFixed(2)}</strong>
          <span id="site-pending-count" style="color:var(--amber);${pendingCount>0?'':'display:none;'}">
            &nbsp;·&nbsp; ${pendingCount} ${lang==='vi'?'chờ duyệt':'pending'}
          </span>
        </p>
      </div>
      <div class="page-actions">
    ...
    <div class="card" style="margin-bottom:var(--gap);background:var(--amber-bg);border-color:var(--amber);">
      <div class="card-body" style="font-size:.84rem;color:var(--amber);">
        ...
        ${lang==='vi'
          ? 'Tổng số giờ làm việc tại công trường (Giờ ra − Giờ vào) lớn hơn 4 tiếng → Phnom Penh: $5/ngày · Tỉnh: $6/ngày. Phụ cấp xăng theo quãng đường được tính riêng, không phụ thuộc điều kiện này.'
          : 'Total hours worked on site (End − Start) greater than 4 hours → Phnom Penh: $5/day · Province: $6/day. Fuel allowance by distance is calculated separately and does not depend on this rule.'}
      </div>
    </div>

    <div class="filter-bar">
      <select class="form-control" id="site-filter-month" onchange="filterSiteRecords()">
        <option value="">${lang==='vi'?'Tất cả các tháng':'All Months'}</option>
        ${siteMonthOptionsHtml}
      </select>
      ...
      <select class="form-control" id="site-filter-employee" onchange="filterSiteRecords()">
        <option value="">${lang==='vi'?'Tất cả nhân viên':'All Staff'}</option>
        ${(state.cache.employees || []).slice().sort((a,b)=>(a.name||'').localeCompare(b.name||'')).map(e=>`<option value="${e.uid}">${e.name}</option>`).join('')}
      </select>
    </div>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            ...
            <th>${lang==='vi'?'Quãng đường':'Distance'}</th>
            <th>${lang==='vi'?'Phụ cấp':'Allowance'}</th>
            <th>${lang==='vi'?'Trạng thái':'Status'}</th>
            <th>${t('emp_actions')}</th>
          </tr></thead>
          <tbody id="site-tbody">
            ${renderSiteRows(defaultRecords, canApprove)}
          </tbody>
        </table>
      </div>
    </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>${t('site_start')}</label>
              ${amPmTimeInputHtml('site-start', '07:30', 'onchange="calcSiteAllowance()"')}
            </div>
            <div class="form-group">
              <label>${t('site_end')}</label>
              ${amPmTimeInputHtml('site-end', '14:00', 'onchange="calcSiteAllowance()"')}
            </div>
          </div>
          <div class="form-group">
            <label>${lang==='vi'?'Quãng đường 1 chiều (Lab → Site, km)':'One-way distance (Lab → Site, km)'}</label>
            <input class="form-control" type="number" id="site-distance" placeholder="0" min="0" step="0.1" oninput="calcSiteAllowance()">
          </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>${t('site_start')}</label>
              ${amPmTimeInputHtml('adm-site-start', '07:30', 'onchange="calcAdminSiteAllowance()"')}
            </div>
            <div class="form-group">
              <label>${t('site_end')}</label>
              ${amPmTimeInputHtml('adm-site-end', '14:00', 'onchange="calcAdminSiteAllowance()"')}
            </div>
          </div>
          <div class="form-group">
            <label>${lang==='vi'?'Quãng đường 1 chiều (Lab → Site, km)':'One-way distance (Lab → Site, km)'}</label>
            <input class="form-control" type="number" id="adm-site-distance" placeholder="0" min="0" step="0.1" oninput="calcAdminSiteAllowance()">
          </div>
...
function renderSiteRows(records, canApprove) {
  ...
  return records.map(r => {
    ...
    const hasAmount = (r.amount || 0) > 0 && r.status !== 'rejected';
    ...
    return `<tr>
      ...
      <td class="td-mono">
        ${r.distanceKm != null ? `${r.distanceKm}km${r.fuelOver50km ? ` <span class="badge badge-amber" style="font-size:.7rem;">${lang==='vi'?'>50km riêng':'>50km separate'}</span>` : ''}` : '–'}
      </td>
      <td class="td-mono ${hasAmount ?'payroll-positive':''}">
        ${hasAmount ? `+$${(r.amount||0).toFixed(2)}` : '–'}
      </td>
      <td>${statusBadge}</td>
      <td style="white-space:nowrap;">
        ${canAct ? `
...
window.filterSiteRecords = function() {
  const month = $('site-filter-month')?.value || '';
  const status = $('site-filter-status')?.value || '';
  const employee = $('site-filter-employee')?.value || '';
  let filtered = _siteRecordsCache;
  if (month) filtered = filtered.filter(r => monthKeyOf(r.date) === month);
  if (status) filtered = filtered.filter(r => r.status === status);
  if (employee) filtered = filtered.filter(r => r.employeeId === employee);
  const canApprove = isAdmin() || isCountryManager();
  const tbody = $('site-tbody');
  if (tbody) tbody.innerHTML = renderSiteRows(filtered, canApprove);

  // Cập nhật tổng phụ cấp đã duyệt + số bản ghi + số chờ duyệt theo bộ lọc hiện tại
  // Lưu ý: tính theo status==='approved', KHÔNG lọc thêm theo r.eligible —
  // vì bản ghi không đủ điều kiện ăn ($5/$6) nhưng có tiền xăng vẫn phải
  // được cộng vào tổng (amount đã gồm cả 2 phần khi lưu).
  const totalEligible = filtered.filter(r => r.status==='approved')
    .reduce((s,r) => s + (r.amount||0), 0);
  const pendingCount = filtered.filter(r => r.status === 'pending').length;
  const totalEl = $('site-total-eligible');
  if (totalEl) totalEl.textContent = `$${totalEligible.toFixed(2)}`;
  const countEl = $('site-record-count');
  if (countEl) countEl.textContent = filtered.length;
  const pendingEl = $('site-pending-count');
  if (pendingEl) {
    if (pendingCount > 0) {
      pendingEl.style.display = '';
      pendingEl.innerHTML = `&nbsp;·&nbsp; ${pendingCount} ${lang==='vi'?'chờ duyệt':'pending'}`;
...
// Phụ cấp xăng theo quãng đường 1 chiều từ lab tới site — theo đúng bậc
// thang công ty quy định. Trên 50km KHÔNG tự động cộng — phải thanh toán
// riêng (trả về null để phân biệt với 0, tránh nhầm "0km = không đủ điều
// kiện" với "trên 50km = cần xử lý riêng").
function calcFuelAllowance(distanceKm){
  ...
  if (km <= 35) return 3;
  if (km <= 50) return 5;
  return null; // > 50km: thanh toán riêng, không tự động tính vào phụ cấp
}

window.calcSiteAllowance = function() {
  const start    = getTime12('site-start');
  const end      = getTime12('site-end');
  const location = $('site-location')?.value || 'Phnom Penh';
  const distance = $('site-distance')?.value || '';
  const eligible = calcSiteEligible(start, end);
  const rate     = SITE_RATES[location] || 5;
  const fuel     = calcFuelAllowance(distance);
  ...
  let html = '';
  if (eligible) {
    html += `✅ <strong>${lang==='vi'?'Đủ điều kiện':'Eligible'}</strong> →
    ...
    html += `❌ <span style="color:var(--text-muted)">
      ${lang==='vi'
        ? 'Không đủ điều kiện — tổng số giờ làm việc cần lớn hơn 4 tiếng'
        : 'Not eligible — total hours worked must be greater than 4 hours'}
    </span>`;
  }
  if (distance) {
    if (fuel === null) {
      html += `<br>⛽ ${lang==='vi'
        ? `Quãng đường ${distance}km — <strong>trên 50km, cần thanh toán riêng</strong> (không tự động cộng vào phụ cấp)`
        : `Distance ${distance}km — <strong>over 50km, must be settled separately</strong> (not auto-added to allowance)`}`;
    } else if (fuel > 0) {
      html += `<br>⛽ ${lang==='vi'?'Phụ cấp xăng':'Fuel allowance'} (${distance}km): <strong class="payroll-positive">+$${fuel}</strong>`;
    }
  }
...
window.calcAdminSiteAllowance = function() {
  const start    = getTime12('adm-site-start');
  const end      = getTime12('adm-site-end');
  const location = $('adm-site-location')?.value || 'Phnom Penh';
  const distance = $('adm-site-distance')?.value || '';
  const eligible = calcSiteEligible(start, end);
  const rate     = SITE_RATES[location] || 5;
  const fuel     = calcFuelAllowance(distance);
  ...
  if (distance) {
    if (fuel === null) {
      html += `<br>⛽ ${lang==='vi'
        ? `Quãng đường ${distance}km — <strong>trên 50km, cần thanh toán riêng</strong>`
        : `Distance ${distance}km — <strong>over 50km, settle separately</strong>`}`;
    } else if (fuel > 0) {
      html += `<br>⛽ ${lang==='vi'?'Phụ cấp xăng':'Fuel allowance'} (${distance}km): <strong class="payroll-positive">+$${fuel}</strong>`;
    }
  }
...
window.openLogSite = function() {
  ...
  initAmPmTimeInputs();
  setTime12('site-start', '07:30');
  setTime12('site-end', '14:00');
  calcSiteAllowance();
};
...
window.openAdminLogSite = function() {
  ...
  initAmPmTimeInputs();
  setTime12('adm-site-start', '07:30');
  setTime12('adm-site-end', '14:00');
...
window.saveSiteRecord = async function() {
  ...
  const eligible = calcSiteEligible(start, end);
  const fuelAllowance = calcFuelAllowance(distance);
  // Trên 50km: KHÔNG cộng tự động (null -> 0 khi lưu), đánh dấu riêng để
  // admin biết cần thanh toán ngoài quy trình phụ cấp thông thường.
  const fuelOver50km = fuelAllowance === null;
  const amount   = (eligible ? (SITE_RATES[location] || 5) : 0) + (fuelOver50km ? 0 : (fuelAllowance || 0));
  ...
  console.log('[Site] Writing:', {
    ...
    country: safeCountry,
    date, location, start, end, eligible, distance, fuelAllowance, amount
  });
  ...
  try {
    ...
    toast(
      lang==='vi'
        ? `✅ Đã gửi ghi nhận đi công trường. Chờ duyệt.${amount > 0 ? ` (Dự kiến: +$${amount})` : ''}${fuelOver50km ? ' — Quãng đường >50km cần thanh toán riêng.' : ''}`
        : `✅ Site visit submitted. Pending approval.${amount > 0 ? ` (Estimated: +$${amount})` : ''}${fuelOver50km ? ' — Distance >50km must be settled separately.' : ''}`,
      'success'
    );
...
window.adminSaveSiteRecord = async function() {
  ...
  try {
    ...
    toast(
      amount > 0
        ? (lang==='vi'?`✅ Đã lưu và duyệt. Phụ cấp: +$${amount}${fuelOver50km?' (quãng đường >50km cần thanh toán riêng)':''}`:`✅ Saved & approved. Allowance: +$${amount}${fuelOver50km?' (distance >50km — settle separately)':''}`)
...
window.generatePayroll = async function() {
  ...
  const weeksInMonth = countWeeksInMonth(monthYear, monthNum - 1);
  ...
  try {
    ...
    const agg = {};
    const ensure = uid => {
      if (!agg[uid]) agg[uid] = { late: 0, violations: 0, otNormal: 0, otHoliday: 0, siteAllowance: 0, fixedAllowance: 0, phoneAllowance: 0, hasManualPhone: false };
    };
    ...
    otSnap.docs.forEach(d => {
      ...
      if (r.otType === 'holiday') agg[r.employeeId].otHoliday += r.otPay || 0;
      else agg[r.employeeId].otNormal += r.otPay || 0;
    });
    siteSnap.docs.forEach(d => {
      const r = d.data(); ensure(r.employeeId);
      agg[r.employeeId].siteAllowance += r.amount || 0;
    });
    // Fixed allowances — monthly frequency only. SỬA LỖI: loại "phone" bị
    // TÍNH 2 LẦN trước đây — vừa cộng vào đây, vừa tự động cộng thêm
    // $1/tuần bên dưới cho MỌI nhân viên USD, bất kể đã có phụ cấp điện
    // thoại nhập tay hay chưa. Giờ tách riêng: loại "phone" nhập tay được
    // dùng làm SỐ CHÍNH THỨC (ưu tiên hơn), không cộng vào fixedAllowance
    // nữa; mức $1/tuần mặc định CHỈ áp dụng cho ai CHƯA có phụ cấp điện
    // thoại nhập tay.
    allowSnap.docs.forEach(d => {
      const r = d.data(); ensure(r.employeeId);
      if (r.frequency !== 'monthly') return;
      if (r.allowanceType === 'phone') {
        agg[r.employeeId].phoneAllowance += r.amount || 0;
        agg[r.employeeId].hasManualPhone = true;
      } else {
        agg[r.employeeId].fixedAllowance += r.amount || 0;
      }
    });
    ...
    for (const e of employees) {
      ...
      // Phone allowance: CHỈ tự tính $1/tuần mặc định nếu nhân viên đó
      // CHƯA có phụ cấp điện thoại nhập tay riêng (tránh cộng 2 lần).
      if (!a.hasManualPhone) {
...
function renderPayroll(employees, existing, month) {
  ...
  $('page-payroll').innerHTML = `
    <div class="page-header">
      <div><h2>${t('page_payroll')}</h2><p>${lang==='vi'?'Tháng':'Month'}: <input type="month" id="payrollMonthPicker" value="${month}" onchange="loadPayroll(this.value)" style="padding:4px 8px;border:1px solid var(--line,#ccc);border-radius:4px;"></p></div>
      <div class="page-actions">
        ${!isLocked ? `<button class="btn btn-gold" onclick="generatePayroll()">${t('pay_generate')}</button>` : ''}
        ${isLocked && isSuperAdmin() ? `<button class="btn btn-outline" onclick="unlockPayroll()">🔓 ${lang==='vi'?'Mở khóa để sửa':'Unlock to Edit'}</button>` : ''}
        <button class="btn btn-outline" onclick="exportPayrollCSV()">${t('pay_export')}</button>
        ${isAdmin() && !isLocked ? `<button class="btn btn-primary" onclick="lockPayroll()">${t('pay_lock')}</button>` : ''}
      </div>
    </div>
