...
// Type code prefixes used for asset/equipment numbering: {TYPE}-{COUNTRY}-{SEQ}
...
const TYPE_CODES = {
  ...
  TECH: { vi: "Thiết bị Kỹ thuật", en: "Technical Equipment" },
  DR: { vi: "Thiết bị khoan / hiện trường", en: "Drilling / Field Equipment" },
...
// an admin assigning the correct role/department).
async function ensureUserProfile(fbUser) {
  ...
  const profile = {
    ...
    role: isSuper ? ROLES.SUPERADMIN : ROLES.EMPLOYEE,
    department: isSuper ? null : null,
    country: null,
    active: true,
    createdAt: nowTs()
  };
...
function hasAnyRole(...roles) {
  return STATE.profile && roles.includes(STATE.profile.role);
}

function isManagerOf(department) {
  if (!STATE.profile) return false;
  if (STATE.profile.role === ROLES.SUPERADMIN || STATE.profile.role === ROLES.ADMIN) return true;
  return STATE.profile.department === department &&
    [ROLES.LAB_MANAGER, ROLES.TECH_MANAGER, ROLES.CAL_MANAGER, ROLES.OFFICE_ADMIN].includes(STATE.profile.role);
}

function canManageEquipment() {
  return hasAnyRole(ROLES.SUPERADMIN, ROLES.ADMIN, ROLES.LAB_MANAGER, ROLES.TECH_MANAGER, ROLES.CAL_MANAGER, ROLES.OFFICE_ADMIN);
}
...
auth.onAuthStateChanged(async (fbUser) => {
  if (fbUser) {
    ...
    try {
      STATE.profile = await ensureUserProfile(fbUser);
    } catch (err) {
      console.error(err);
...
function renderUserChip() {
  ...
  document.getElementById("userRole").textContent = L(ROLE_LABELS[STATE.profile.role] || {});
}
...
const NAV_ITEMS = [
  { id: "dashboard", icon: "📊", label: "nav.dashboard", roles: "all" },
  { id: "fixedAssets", icon: "🏢", label: "nav.fixedAssets", roles: "all" },
  { id: "equipment", icon: "🛠️", label: "nav.equipment", roles: "all" },
  { id: "calibration", icon: "📐", label: "nav.calibration", roles: "all" },
  { id: "inventory", icon: "📦", label: "nav.inventory", roles: "all" },
  { id: "movements", icon: "🚚", label: "nav.movements", roles: "all" },
  { id: "maintenance", icon: "🔧", label: "nav.maintenance", roles: "all" },
  { id: "purchase", icon: "🧾", label: "nav.purchase", roles: "all" },
  { id: "reports", icon: "📑", label: "nav.reports", roles: [ROLES.SUPERADMIN, ROLES.ADMIN, ROLES.LAB_MANAGER, ROLES.TECH_MANAGER, ROLES.CAL_MANAGER, ROLES.OFFICE_ADMIN] },
  { id: "users", icon: "👤", label: "nav.users", roles: [ROLES.SUPERADMIN, ROLES.ADMIN] }
];
...
function buildNav() {
  ...
  NAV_ITEMS.forEach((item) => {
    if (item.roles !== "all" && !item.roles.includes(STATE.profile.role)) return;
...
async function generateAssetCode(typeCode, countryCode) {
  ...
  return `${typeCode}-${countryCode}-${seqStr}`;
}
...
PAGE_RENDERERS.dashboard = function renderDashboard() {
  ...
  const visibleAssets = filterByScope(allAssets);
  ...
  const calDue = STATE.cache.calibrationRecords.filter((r) => {
  ...
  const html = `
    ...
    <div class="panel">
      ...
      <div class="panel-body">${renderCalibrationAlertsTable()}</div>
    </div>
...
function filterByScope(items) {
  ...
  if ([ROLES.LAB_MANAGER, ROLES.TECH_MANAGER, ROLES.CAL_MANAGER, ROLES.OFFICE_ADMIN].includes(STATE.profile.role)) {
    const dept = STATE.profile.department || ROLE_DEPARTMENT[STATE.profile.role];
    return items.filter((i) => !i.department || i.department === dept);
  }
  // employee: only items related to them (assigned equipment, own requests)
...
function renderCalibrationAlertsTable() {
  const records = STATE.cache.calibrationRecords
  ...
  return `<div class="table-wrap"><table class="data-table">
    <thead><tr><th>${t("common.code")}</th><th>${STATE.lang === "vi" ? "Hết hạn" : "Expiry"}</th><th>${t("common.status")}</th><th>${STATE.lang === "vi" ? "Số chứng chỉ" : "Certificate"}</th></tr></thead>
          ...
          ${items.length === 0
            ...
            : `<div class="table-wrap"><table class="data-table">
                <thead><tr>
                  ...
                  <th>${t("common.location")}</th><th>${t("common.actions")}</th>
                </tr></thead>
                <tbody>${rows}</tbody>
              </table></div>`
          }
  ...
  async function saveAssetForm(existing, ref, cacheKeyName) {
    ...
    try {
      if (existing) {
        ...
        data.assetCode = await generateAssetCode(typeCode, country);
...
makeAssetModule({
  ...
  defaultTypeCode: "LAB",
  typeCodeChoices: ["LAB", "TECH", "DR", "OFF"],
...
function getCalibratableAssets() {
  return [...STATE.cache.equipment, ...STATE.cache.calibrationStandards].map((a) => ({ id: a.id, assetCode: a.assetCode, name: a.name }));
}

function renderCalibrationRecordsTable() {
  const records = filterByScope(STATE.cache.calibrationRecords).sort((a, b) => (daysUntil(a.expiryDate) || 0) - (daysUntil(b.expiryDate) || 0));
...
function openCalibrationRecordForm(existing) {
  ...
  const optionsHtml = assets.map((a) => `<option value="${a.id}" data-code="${escapeHtml(a.assetCode || "")}" ${existing && existing.assetId === a.id ? "selected" : ""}>${escapeHtml(a.assetCode || "")} - ${escapeHtml(a.name || "")}</option>`).join("");
  const bodyHtml = `
    <div class="field">
...
function openPurchaseForm() {
  ...
  const bodyHtml = `
    <div class="field">
      <label>${STATE.lang === "vi" ? "Chọn vật tư có sẵn (tùy chọn)" : "Pick existing item (optional)"}</label>
      <select id="pr_existing">
        <option value="">${STATE.lang === "vi" ? "-- Vật tư mới / khác --" : "-- New / other item --"}</option>
        ${invItems.map((i) => `<option value="${i.id}">${escapeHtml(i.itemCode)} - ${escapeHtml(i.name)}</option>`).join("")}
      </select>
    </div>
  ...
  function checkStock() {
    ...
    warnBox.innerHTML = `<div class="field-hint" style="background:var(--warning-light);color:var(--warning);padding:8px 10px;border-radius:6px;">
      ${STATE.lang === "vi" ? "Tồn kho hiện có" : "Existing stock"}: ${STATE.lang === "vi" ? "Kho" : "Warehouse"} ${warehouse} + ${STATE.lang === "vi" ? "công trình khác" : "other projects"} ${otherProjects} = <b>${total}</b>.
      ${total > 0 ? (STATE.lang === "vi" ? " Có thể không cần mua thêm." : " Purchase may not be necessary.") : ""}
    </div>`;
  }
...
PAGE_RENDERERS.users = function renderUsers() {
  ...
  const rows = users.map((u) => `<tr>
    ...
    <td>${L(ROLE_LABELS[u.role] || {})}</td>
    <td>${u.department ? L(DEPARTMENTS[u.department] || {}) : "-"}</td>
  ...
  document.getElementById("pageContent").innerHTML = `
    <div class="panel">
      ...
      <div class="panel-body">
        <div class="table-wrap"><table class="data-table">
          <thead><tr><th>${t("common.name")}</th><th>Email</th><th>${STATE.lang === "vi" ? "Vai trò" : "Role"}</th>
          <th>${t("common.department")}</th><th>${t("common.status")}</th><th>${t("common.actions")}</th></tr></thead>
          <tbody>${rows}</tbody></table></div>
      </div>
    </div>
  `;
...
function openUserForm(existing) {
  const roleOptions = Object.keys(ROLE_LABELS).filter((r) => r !== "superadmin" || (existing && existing.role === "superadmin"))
    .map((r) => `<option value="${r}" ${existing && existing.role === r ? "selected" : ""}>${L(ROLE_LABELS[r])}</option>`).join("");
  const deptOptions = Object.keys(DEPARTMENTS).map((d) => `<option value="${d}" ${existing && existing.department === d ? "selected" : ""}>${L(DEPARTMENTS[d])}</option>`).join("");
  ...
  const bodyHtml = existing ? `
    ...
    <div class="field-row">
      <div class="field"><label>${STATE.lang === "vi" ? "Vai trò" : "Role"} *</label><select id="us_role" ${existing.role === "superadmin" ? "disabled" : ""}>${roleOptions}</select></div>
      <div class="field"><label>${t("common.department")}</label><select id="us_dept"><option value="">-</option>${deptOptions}</select></div>
    </div>
  ...
  ` : `
    ...
    <div class="field-row">
      <div class="field"><label>${STATE.lang === "vi" ? "Vai trò" : "Role"} *</label><select id="us_role">${roleOptions}</select></div>
      <div class="field"><label>${t("common.department")}</label><select id="us_dept"><option value="">-</option>${deptOptions}</select></div>
    </div>
  `;
    ...
    if (existing) {
      try {
        await COLLECTIONS.users.doc(existing.id).update({
          ...
          role: document.getElementById("us_role").disabled ? existing.role : document.getElementById("us_role").value,
          department: document.getElementById("us_dept").value || null,
...
function exportAssetsExcel(items, label) {
  const rows = items.map((i) => ({
    ...
    Country: i.country, Brand: i.brand || "", Model: i.model || "", Serial: i.serialNumber || "",
...
function exportAssetsPdf(items, label) {
  ...
  doc.autoTable({
    startY: 20,
    head: [["Code", "Name", "Category", "Country", "Status", "Condition", "Location"]],
    body: items.map((i) => [
      i.assetCode || "", i.name || "", i.category || "", i.country || "",
