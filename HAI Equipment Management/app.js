...
function renderScopeFilterBar() {
  return `
    <div class="field-row" style="margin-bottom:14px;align-items:flex-end;">
      ...
      <div class="field" style="max-width:220px;margin-bottom:0;">
        <label>${STATE.lang === "vi" ? "Sắp xếp theo mã" : "Sort by code"}</label>
        <select id="filterSortCode">
          <option value="" ${!STATE.filters.sortCode ? "selected" : ""}>${STATE.lang === "vi" ? "Mặc định" : "Default"}</option>
          <option value="asc" ${STATE.filters.sortCode === "asc" ? "selected" : ""}>${STATE.lang === "vi" ? "Mã tăng dần" : "Code ascending"}</option>
          <option value="desc" ${STATE.filters.sortCode === "desc" ? "selected" : ""}>${STATE.lang === "vi" ? "Mã giảm dần" : "Code descending"}</option>
        </select>
      </div>
    </div>
  `;
...
function attachScopeFilterHandlers() {
  ...
  const sortSel = document.getElementById("filterSortCode");
  if (sortSel) sortSel.addEventListener("change", (e) => {
    STATE.filters.sortCode = e.target.value;
    if (PAGE_RENDERERS[STATE.page]) PAGE_RENDERERS[STATE.page]();
  });
}
...
function applySortCode(items) {
  if (!STATE.filters.sortCode) return items;
  const sorted = [...items].sort((a, b) => (a.assetCode || "").localeCompare(b.assetCode || "", undefined, { numeric: true }));
  return STATE.filters.sortCode === "desc" ? sorted.reverse() : sorted;
}
...
function makeAssetModule(opts) {
  ...
  PAGE_RENDERERS[pageId] = function render() {
    ...
    const items = applySortCode(applyScopeFilter(getItems()));
  ...
  function openAssetForm(existing) {
    ...
    const bodyHtml = `
      ...
      <div class="field">
        ...
        ${isEdit
          ? `<input id="f_assetCode" value="${escapeHtml(existing.assetCode || "")}">`
          : `<div class="code-preview" id="f_codePreview">${STATE.lang === "vi" ? "Tự động sinh khi lưu" : "Auto-generated on save"}</div>`
        }
      </div>
      <div class="field-row">
  ...
  async function saveAssetForm(existing, ref, cacheKeyName) {
    ...
    try {
      ...
      if (existing) {
        const newCode = document.getElementById("f_assetCode").value.trim();
        if (!newCode) { showToast(t("common.requiredField"), "error"); return; }
        if (newCode !== existing.assetCode) {
          // Kiểm tra trùng mã trước khi lưu — tránh 2 thiết bị cùng 1 mã.
          const dup = await ref.where("assetCode", "==", newCode).get();
          if (!dup.empty && dup.docs.some((d) => d.id !== existing.id)) {
            showToast(STATE.lang === "vi" ? "Mã này đã được dùng cho thiết bị khác" : "This code is already used by another item", "error");
            return;
          }
        }
        assetCode = newCode;
